...
# Define paths and globals
OLLAMA_EXE_PATH = "ollama"  # Assuming this is in the PATH if installed
OLLAMA_RUNNERS_DIR = os.path.join(os.getcwd(), "ollama", "ollama_runners")
OLLAMA_TEMP_DIR = os.path.join(os.getcwd(), "ollama_temp")
OLLAMA_ZIP_PATH = os.path.join(os.getcwd(), "Ollama-darwin.zip")
OLLAMA_PROCESS = None
...
def install_ollama():
    """Install the Ollama software on macOS."""
    ...
    final_path = shutil.move(os.path.join(OLLAMA_TEMP_DIR, "ollama"), OLLAMA_EXE_PATH)
        ...
        try:
            if 'ollama' in process.info['name'].lower() and process.info['username'] == os.getlogin():
                ...
                try:
                    ...
                    print(f"Process {process.info['pid']} does not exist anymore.")
                except psutil.TimeoutExpired:
                    print(f"Process {process.info['pid']} did not terminate in time. Killing it forcefully.")
                    process.kill()
                    process.wait(timeout=5)
...
def clear_gpu_memory():
    """Print a message about GPU memory clearing."""
    #print("GPU memory clearing is only implemented for Windows with Nvidia GPUs.")
    print("Ollama AI local data generation complete! Continuing...")
...
def is_ollama_installed():
    """Check if Ollama is installed by running 'ollama help'."""
    try:
        result = subprocess.run([OLLAMA_EXE_PATH, "help"], capture_output=True, text=True, check=True)
        return True
